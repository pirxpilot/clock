...
const format = {
  ...
  table(caption, type, tbody) {
    return el('table', el('caption', caption) + el('tbody', tbody), { class: type });
  }
};

function renderTable(caption, type, rows, attrs) {
  const tbody = rows.map(row => el('tr', row.join(''), attrs)).join('');
  return format.table(caption, type, tbody);
}

function renderHours(captions) {
  const hours = Array.from({ length: 24 }, (_, hour) => format.cell(hour, 'hour'));
...
function renderMinutes(captions) {
  const minutes = Array.from({ length: 60 / minuteStep }, (_, minute) => format.cell(minute * minuteStep, 'minute'));
...
export default class Clock extends Emitter {
  ...
  onclick(e) {
    ...
    if (!a.classList.contains('invalid')) {
      this.selected[kind] = Number.parseInt(value, 10);
  ...
  markInvalid(selectedHour, both) {
    ...
    if (both) {
      markInvalid(this.querySelectorAll('hour', 'a'), 'hour', hour => valid.isValidHour(hour));
    }
    markInvalid(this.querySelectorAll('minute', 'a'), 'minute', minute => valid.isValidMinute(selectedHour, minute));
  }
